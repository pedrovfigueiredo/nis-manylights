...
inline PBRT_CPU_GPU void RecordShadowRayResult(const ShadowRayWorkItem w,
    ...
    if (foundIntersection) {
        ...
        // Update the light sampler's state if the shadow ray was occluded.
        if (w.isBasicBSDF){
            if (lightSampler.isTraining(w.pixelIndex, w.depth) && w.isVPL)
                    lightSamplerOptStateBuffer->recordRadiance(w.pixelIndex, SampledSpectrum(0.f), 1.f);
        }
        
        return;
    }
    const Float Ld_denom = (w.r_u + w.r_l).Average();
    SampledSpectrum Ld = Ld_denom > 0.f ? w.Ld / Ld_denom : SampledSpectrum(0.f);
...
inline PBRT_CPU_GPU void TraceTransmittance(ShadowRayWorkItem sr,
    ...
    if (T_ray) {
        ...
        SampledSpectrum LdCurr = T_ray;
        Float Ld_denom = (sr.r_u * r_u + sr.r_l * r_l).Average();
        Ld *= LdCurr / Ld_denom;
        ...
        // Update the light sampler's state
        if (lightSampler.isTraining(sr.pixelIndex, sr.depth))
            lightSamplerOptStateBuffer->recordRadiance(sr.pixelIndex, LdCurr, Ld_denom);
    }
}
