...
PBRT_CPU_GPU inline bool IsDeltaLight(LightType type) {
    return (type == LightType::VPL || type == LightType::DeltaPosition || type == LightType::DeltaDirection);
}
...
class LightSampleContext {
  public:
    ...
    PBRT_CPU_GPU
    LightSampleContext(const SurfaceInteraction &si, int pixelIndex)
        : pi(si.pi), n(si.n), ns(si.shading.n), wo(si.wo), pixelIndex(pixelIndex) {}
    PBRT_CPU_GPU
    LightSampleContext(const Interaction &intr, int pixelIndex = -1) : pi(intr.pi), n(intr.n), ns(intr.n), wo(intr.wo), pixelIndex(pixelIndex) {}
    PBRT_CPU_GPU
    LightSampleContext(Point3fi pi, Normal3f n, Normal3f ns, Vector3f wo, int pixelIndex) : pi(pi), n(n), ns(ns), wo(wo), pixelIndex(pixelIndex) {}
    PBRT_CPU_GPU
    LightSampleContext(Point3f p, Normal3f n, Normal3f ns, Vector3f wo, int pixelIndex) : pi(p), n(n), ns(ns), wo(wo), pixelIndex(pixelIndex) {}
...
class VirtualPointLight : public LightBase {
  public:
    VirtualPointLight(Transform renderFromLight, MediumInterface mediumInterface,
                      const Point3f &pos, const Normal3f &normal, Float radius,
                      Spectrum I, Float scale)
        : LightBase(LightType::VPL, renderFromLight, mediumInterface),
          p(pos),
          n(Normalize(normal)),
          reps(radius),
          I(LookupSpectrum(I)),
    ...
    void Preprocess(const Bounds3f &sceneBounds) {}
    std::string ToString() const;
    static VirtualPointLight *Create(const Transform &renderFromLight, Medium medium,
                                     const ParameterDictionary &parameters,
                                     const RGBColorSpace *colorSpace, const FileLoc *loc,
                                     Allocator alloc);
    ...
    pstd::optional<LightLiSample> SampleLi(LightSampleContext ctx, Point2f u,
        ...
        // Compute geometric term with clamping
        Float cosTheta = AbsDot(-wi, n);
        Float distSqr = DistanceSquared(p, ctx.p());
        Float gTerm = cosTheta / distSqr;
        gTerm = std::min(gTerm, 32.0f);  // Clamp to prevent singularities

        // SampledSpectrum Li = scale * I->Sample(lambda) / distSqr; <- Match point light effect
        SampledSpectrum Li = scale * I->Sample(lambda) * gTerm;
        return LightLiSample(Li, wi, 1, Interaction(p, &mediumInterface));
    }

    PBRT_CPU_GPU
    Float PDF_Li(LightSampleContext, Vector3f, bool allowIncompletePDF) const {
        return 0;  // Delta light - no PDF needed
    }

    PBRT_CPU_GPU
    pstd::optional<LightLeSample> SampleLe(Point2f u1, Point2f u2,
                                           SampledWavelengths &lambda, Float time) const {
        Frame frame = Frame::FromZ(Vector3f(n));
        Point2f u = u1;
        Float z = std::sqrt(1 - u[0]);
        Float r = std::sqrt(u[0]);
        Float phi = 2 * Pi * u[1];
        Vector3f local(r * std::cos(phi), r * std::sin(phi), z);
        Vector3f w = frame.FromLocal(local);

        Ray ray(p, w, time, mediumInterface.outside);
        Float pdf = z / Pi;
        return LightLeSample(scale * I->Sample(lambda), ray, 1, pdf);
    }

    PBRT_CPU_GPU
    void PDF_Le(const Ray &ray, Float *pdfPos, Float *pdfDir) const {
        *pdfPos = 0;
        *pdfDir = AbsDot(ray.d, n) / Pi;
    }
    ...
    SampledSpectrum Phi(SampledWavelengths lambda) const {
        return 2 * Pi * scale * I->Sample(lambda);
    }

    pstd::optional<LightBounds> Bounds() const {
        Float phi = 2 * Pi * scale * I->MaxValue();
        return LightBounds(Bounds3f(p, p), Vector3f(n), phi,
                           std::cos(Pi),      // Full hemisphere coverage
                           std::cos(Pi / 2),  // Hemispherical emission
                           false);            // Not two-sided
    }
    // VirtualPointLight Private Members
    const Point3f p;                  // Position
    const Normal3f n;                 // Surface normal
    const Float reps;                 // Radius of effect
    const DenselySampledSpectrum *I;  // Intensity (using PBRT's spectrum cache)
